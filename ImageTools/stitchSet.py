...
class StitchSet():
    ...
    def exportRGBAImage(self):
        mask = np.array(self._bands['red'].mask, copy=True).astype(float)
        mask = np.place(mask, mask==True, 0)
        mask = np.place(mask, mask==False, 255)
        bands = np.stack((self._bands['red'], self._bands['green'], self._bands['blue'],mask))
        bands = bands.astype(float)*255.0/655335.0
        tif_path = os.path.join(self._output_directory, '{}_rgba.tif'.format(self._output_base,index))
        return saveColorGeoTiff(bands, 'rgba', tif_path)
    
    def saveGeoTiff(self, bands, index, filepath, dtype, options, fill=False):  # bands is list of np arrays or a 3d np array
        num_bands = len(bands)
    ...
    def saveColorGeoTiff(self, bands, index, filepath):
        dtype = gdal.GDT_Byte
        return self.saveGeoTiff(bands, index, filepath, dtype, ['PROFILE=GeoTIFF','GTIFF_FORCE_RGBA=YES'])
    
    def saveMonoGeoTiff(self, band, index, filepath):
        dtype = gdal.GDT_Float32
        return self.saveGeoTiff([band],index, filepath, dtype, ['PROFILE=GeoTIFF'], True)
    ...
    def exportColorImage(self, index, colormap_name, label_name):
        ...
        self.saveColorGeoTiff(colormapped_data, index, tif_path)
